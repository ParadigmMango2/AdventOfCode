...
public class MainRetrospect {
    ...
    private static String getInputsPath() {
        String[] packageArr = MainRetrospect.class.getName().split("\\.");
    ...
    public static long cachedNumPaths (List<List<Character>> input, int beam, int depth) {
        Pair<Integer, Integer> key = new Pair<>(beam, depth);

        if (cache.containsKey(key)) return cache.get(key);

        long result = numPaths(input, beam, depth);
    ...
    public static long numPaths(List<List<Character>> input, int beam, int depth) {
        ...
        if (input.get(depth).get(beam) == '^') {
            ...
            if (beam > 0)
                total += cachedNumPaths(input, beam - 1, depth + 2);
            if (beam < input.getFirst().size() - 1)
                total += cachedNumPaths(input, beam + 1, depth + 2);

            return total;
        } else {
            return cachedNumPaths(input, beam, depth + 2);
        }
    }

    public static void part2Recursive() {
        var input = parse1(getInputsPath() + "real.txt");

        var firstLine = input.getFirst();
        int firstBeamIdx = firstLine.indexOf('S');

        System.out.println(numPaths(input, firstBeamIdx, 2));
    }
